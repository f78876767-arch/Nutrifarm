...
<div class="min-h-screen bg-gray-50 py-8">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    @if(session('success'))<div class="mb-4 p-3 rounded bg-emerald-100 text-emerald-800 text-sm">{{ session('success') }}</div>@endif
    @if(session('warning'))
      <div class="mb-4 p-3 rounded bg-yellow-100 text-yellow-800 text-sm">{{ session('warning') }}</div>
      @if(session('import_errors'))
        <div class="mb-4 bg-white border border-yellow-300 rounded p-3 max-h-48 overflow-y-auto text-xs space-y-1">
      ...
      @endif
    @endif
    @if(session('error'))<div class="mb-4 p-3 rounded bg-red-100 text-red-800 text-sm">{{ session('error') }}</div>@endif
      ...
      <div class="px-4 py-5 sm:p-6">
        <div class="flex justify-between items-center mb-6">
          ...
          <div class="flex space-x-3">
            <a href="{{ route('admin.bulk-products.import-template') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium">Download Excel Template</a>
            <button onclick="document.getElementById('importModal').style.display='block'" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md text-sm font-medium">Import Products</button>
            <a href="{{ route('admin.bulk-products.export') }}" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-md text-sm font-medium">Export Products</a>
          </div>
        </div>
...
<div id="importModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
  <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
    <div class="mt-3 text-center">
      ...
      <p class="text-xs text-gray-500 mb-4 text-left leading-relaxed">
        1. Download template, isi kolom.<br>
        2. image_mode: "url" atau "filename".<br>
        3. Jika URL: isi image_1..3 dengan link gambar langsung.<br>
        4. Jika filename: isi nama file saja dan upload ZIP berisi file-file itu.<br>
        5. Satu baris = satu variant. Produk sama (kolom name) akan ditambahkan variant baru.<br>
      </p>
      <form action="{{ route('admin.bulk-products.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Excel File (.xlsx/.csv)</label>
          <input type="file" name="file" accept=".xlsx,.csv,.xls" required class="block w-full text-sm text-gray-600 border rounded p-1" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">ZIP Images (opsional)</label>
          <input type="file" name="images_zip" accept=".zip" class="block w-full text-sm text-gray-600 border rounded p-1" />
        </div>
        <div class="pt-2">
          <button type="submit" class="w-full bg-green-600 hover:bg-green-700 text-white py-2 rounded-md text-sm font-medium">Import</button>
          <button type="button" onclick="document.getElementById('importModal').style.display='none'" class="mt-2 w-full bg-gray-500 hover:bg-gray-600 text-white py-2 rounded-md text-sm font-medium">Cancel</button>
        </div>
      </form>
    </div>
  </div>
